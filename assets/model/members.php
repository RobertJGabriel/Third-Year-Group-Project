...
$idMaker = 0;
        if($count!=0){
            while ($row = $result->fetch_assoc()) {
                ...
                echo '      <img class="trainerImg" src="assets/img/profilePhotos/' .$row['memberId'] . '.png" width="180" height="280" />';
                echo '  </div>';
                echo '</li>';
				$idMaker++;
...
// -- Purpose : Gets all trainers from the database.
    ...
    function getSingleTrainers($trainer){
	  ...
	  echo'<div><img id="trainerBookingImg" src="assets/img/profilePhotos/' . $trainer.  '.png" /></div>';
